...
FROM 
    TRANSAKSI tr
JOIN 
    JADWAL j ON tr.ID_JADWAL = j.ID_JADWAL
JOIN 
    FILM f ON j.ID_FILM = f.ID_FILM
JOIN 
    THEATER th ON j.ID_THEATER = th.ID_THEATER
GROUP BY 
    th.ID_THEATER, 
    f.JUDUL
HAVING 
...
SELECT JUDUL_FILM, TOTAL_TIKET_TERJUAL
FROM (
    SELECT 
        f.JUDUL AS JUDUL_FILM, 
